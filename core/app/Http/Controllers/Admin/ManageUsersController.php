...
class ManageUsersController extends Controller
{
    ...
    public function documentunverfied()
    {
        $page_title = 'Manage all Users Unverified Document';
        $empty_message = 'No Unverified Document  user found';
        $users = User::where('id_verification_doc',0)->orwhere('id_verification_doc',3)->orwhere('id_verification_address',0)->orwhere('id_verification_address',3)->orderBy('id','desc')->paginate(config('constants.table.default'));
        return view('admin.users.users', compact('page_title', 'empty_message', 'users'));
    }
    ...
    public function search(Request $request, $scope)
    {
        ...
        switch ($scope) {
            ...
            case 'documentUnverified':
                $page_title .= 'Document Unverified ';
                $users = where('id_verification_doc',0)->orwhere('id_verification_doc',3)->orwhere('id_verification_address',0)->orwhere('id_verification_address',3);
                break;
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $user->update([
            ...
            'ts'        => $request->ts ? 1 : 0,
            'tv'        => $request->tv ? 1 : 0,
           // 'id_verification_doc'        =>   $id_verification_doc,
    ...
    public function identityVerification($userid ,$status){
        ...
        if($status==1){
           ...
           $user->update([
                       'id_verification_doc'  =>1,
                       'id_verification_address'=>1,

                ]);

            $admin_subject ='The document has been verified. ';
            $admin_message = 'Your document has been Verified thank you for applying ';
            send_general_email($user->email,  $admin_subject, $admin_message, $user->username);
        ...
        elseif($status==0){
            ...
            $user->update([
                'id_verification_doc'   => NULL,
                'id_verification_address'=>NULL,
                'first_name_doc'      => '',
                'middle_name_doc'     => '',
                'last_name_doc'       =>'',
                'date_of_birth_doc'   =>'',
                ...
                'country_doc'         => '',
                'verification_doc'    =>'',
                'verification_doc_back'=>'',
                'verification_address_doc'=>'',
               ]);

           $subject ='The document has been Rejected';
           $message = 'This document is  reject please try  again with new document  ';
           send_general_email($user->email,  $subject, $message, $user->username);


        }

        // $id_verification_doc = $request->id_verification_doc;
        // $id_verification_address = $request->id_verification_address;

        // // if($user->id_verification_doc == 1){
        ...
        // // }
        // // if($user->id_verification_doc == 0){

        // //     $id_verification_doc = 1;

        // // }

        // if($user->id_verification_address == 1){

        //     $user->update([
        //         'verification_address_doc'=>'NULL'
        //     ]);



        // }
        // if($user->id_verification_address == 0){
        ...
        $notify[] = ['success', 'Identity Information has been updated'];
        return redirect()->route('admin.users.detail', $user->id)->withNotify($notify);
