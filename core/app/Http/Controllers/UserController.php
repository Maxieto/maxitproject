...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        $gn       = GeneralSetting::where('id_verification_doc',1)->orWhere('id_verification_address',1)->first();
        $document = User::where('id',Auth::id())->first();

        $page_title = "Identity Verification And Address Verification ";


        if( ($gn && $document->id_verification_doc == 0)  || ($gn && $document->id_verification_doc == 'NULL') || ($gn && $document->id_verification_address == 0) || ($gn && $document->id_verification_address == "NULL") ){


            return view(activeTemplate() . 'user.auth.authorize_document', compact('page_title','document'),);
        }else{
    ...
    public function refMy()
    {
      ...
      public function document_store(Request $request){
        ...
        $request->validate([
                'verification_doc'                 => 'required|image|  mimes:jpeg,png,jpg,gif | max:10248',
                'verification_doc_back'            => 'nullable|image|  mimes:jpeg,png,jpg,gif | max:10248',
                'verification_address_doc'         => 'required|image|  mimes:jpeg,png,jpg,gif | max:10248',
                'first_name_doc'                   => 'required',
                'middle_name_doc'                  => 'required',
                'last_name_doc'                    => 'required',
                'date_of_birth_doc'                => 'required',
                'address_doc'                      => 'required',
                'city_doc'                        => 'required',
                'country_doc'                     => 'required',
        ...
        if ($request->hasFile('verification_doc')) {
               ...
               $verification_doc=$request->file('verification_doc');
                $filename = rand(1000, 9999) . time() . '.' . $verification_doc->getClientOriginalExtension();
                $verification_doc->move('assets/images/doc', $filename);

            }
        if ($request->hasFile('verification_address_doc')) {
            $verification_address_doc=$request->file('verification_address_doc');
            $filename1 = rand(1000, 9999) . time() . '.' . $verification_address_doc->getClientOriginalExtension();
            $verification_address_doc->move('assets/images/doc', $filename1);
            }
        if ($request->hasFile('verification_doc_back')) {
            $verification_doc_back=$request->file('verification_doc_back');
            $filename2= rand(1000, 9999) . time() . '.' . $verification_doc_back->getClientOriginalExtension();
            $verification_doc_back->move('assets/images/doc', $filename2);
            }

            $user = User::find( Auth::id());
            $user->id_verification_doc      = 0;
            $user->id_verification_address  = 0;
            $user->first_name_doc           = $request->first_name_doc;
            $user->middle_name_doc          = $request->middle_name_doc;
            $user->last_name_doc            = $request->last_name_doc ;
            $user->address_doc              = $request->address_doc ;
            $user->city_doc                 = $request->city_doc ;
            $user->country_doc              = $request->country_doc ;
            $user->date_of_birth_doc        =  date('Y-m-d', strtotime($request->date_of_birth_doc));
            $user->verification_doc         =   $filename;
            $user->verification_address_doc = $filename1;
            $user->verification_doc_back    = $filename2 ?? '';
            $user->save();

            $subject = " Document submission  confirmation ";
            $message = " Document Verification   is in process";

            send_general_email($user->email, $subject, $message, $user->username);

           $admin = Admin::find(1);

            $admin_subject ='New user submit document';
            $admin_message = 'A new user has just submitted a document Details go to Admin dashboard';

            send_general_email($admin->email,  $admin_subject, $admin_message, $user->username);
            $notify[] = ['success', 'Document Verification Submitted  Successfully'];
            return back()->withNotify($notify);
